...
db.on('connect', () => console.log('✅ Connected to TrustMicro Supabase.'));
...
// LOGIN (Matches api.ts /auth/login)
...
// DEACTIVATE (Matches api.ts /auth/deactivate)
...
// SIGN-UP (Matches api.ts /auth/signup)
...
app.post('/api/v1/loans', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        await db.query(query, [`LOAN-${Date.now()}`, loan.customerName, loan.bvn, loan.nin, loan.phone, loan.loanAmount || 0, loan.loanAmount || 0, 'Pending', officerEmail, new Date().toISOString().split('T')[0], loan.bankName, loan.accountNumber, loan.employerName || 'N/A']);
...
app.post('/api/v1/manager/reactivate-staff', authenticateToken, async (req, res) => {
    ...
    try {
        await db.query("UPDATE staff_users SET is_active = true, failed_attempts = 0 WHERE LOWER(TRIM(email)) = $1", [req.body.staffEmail.toLowerCase()]);
