...
// Log every request to help us debug in Render logs
...
// --- 4. AUTH ROUTES (Aligned with api.ts baseURL) ---
...
// DEACTIVATE ROUTE (The one causing the 404)
app.post('/api/v1/auth/deactivate', async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        // This query matches your DB columns: is_active and failed_attempts
        const query = `
            UPDATE staff_users 
        ...
        if (result.rowCount === 0) return res.status(404).json({ error: "Staff email not found." });
...
app.post('/api/v1/auth/signup', async (req, res) => {
    ...
    try {
        ...
        await db.query(query, [full_name, email.trim().toLowerCase(), phone_no, hashedPassword, role || 'Officer', branch, true, 0]);
...
app.post('/api/v1/loans', authenticateToken, async (req, res) => {
    try {
        ...
        const values = [`LOAN-${Date.now()}`, loan.customerName, loan.bvn, loan.nin, loan.phone, loan.loanAmount || 0, loan.loanAmount || 0, 'Pending', req.user.email.toLowerCase(), new Date().toISOString().split('T')[0], loan.bankName, loan.accountNumber, loan.employerName || 'N/A'];
...
app.get('/', (req, res) => res.send("🚀 TrustMicro Secure API is Live!"));
