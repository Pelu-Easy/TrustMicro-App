...
//     // --- FIXED SELF-HEALING: ADD createdAt IF MISSING ---
//     db.all("PRAGMA table_info(loans)", (err, columns) => {
//         if (err) return;
//         const hasCreatedAt = columns.some(col => col.name === 'createdAt');
//         if (!hasCreatedAt) {
//             console.log("⚠️ Missing 'createdAt' column. Applying fix...");
//             // We use a simple ALTER without the CURRENT_TIMESTAMP function to satisfy SQLite constraints
//             db.run("ALTER TABLE loans ADD COLUMN createdAt DATETIME", (err) => {
//                 if (err) console.error("❌ Failed to add column:", err.message);
//                 else console.log("✅ 'createdAt' column added successfully.");
//             });
//         }
//     });
// });
...
//             const hashedPassword = await bcrypt.hash(password, 10);
//             const query = `INSERT INTO users (full_name, email, phone_no, password_hash, role, branch, department, unit, supervisor, is_supervisor) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
//             db.run(query, [fullName, email, phone, hashedPassword, role || 'Officer', branch, department, unit, supervisor, is_supervisor || 0], function(err) {
...
//     // We manually add the timestamp here to be safe
//     const now = new Date().toISOString();
...
//     if (userRole === 'manager' || userRole === 'supervisor' || is_supervisor == 1) {
//         query = "SELECT * FROM loans ORDER BY id DESC"; // Fallback to id if createdAt is null for old records
//     } else {
//         query = "SELECT * FROM loans WHERE createdByEmail = ? ORDER BY id DESC";
//         params = [email];
//     }
